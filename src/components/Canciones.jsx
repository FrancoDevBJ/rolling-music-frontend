...
const Canciones = ({ songs, onRemoveSong, isPlaylistView = false }) => {
    ...
    // ✅ Solo las canciones reproducibles forman la cola
    const playableSongs = songs.filter(s => !!s.preview || !!s.url_cancion || !!s.audio);
        ...
        <div className="mb-10 sm:mb-12 px-2 sm:px-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6
                ...
                {songs.map((item, index) => {
                    ...
                    const handleClick = () => {
                        if (isPlayable) {
                            // ✅ Pasa la cola completa de canciones reproducibles
                            selectSong(item, playableSongs);
                        } else {
                            navigate("/notFound");
                        }
                    };
                    ...
                    return (
                        ...
                        >
                            ...
                            {/* QUITAR / AGREGAR A PLAYLIST */}
                            {isPlaylistView ? (
                                <button
                                    onClick={(e) => { e.stopPropagation(); onRemoveSong(item.codigo_unico); }}
                                ...
                                <button
                                    onClick={(e) => { e.stopPropagation(); setSelectedSongForPlaylist(item); }}
                            ...
                            {/* MENU MOBILE */}
                            <button
                                onClick={(e) => { e.stopPropagation(); setMobileMenuSong(item); }}
                            ...
                            {mobileMenuSong && (
                                <div className="fixed inset-0 z-50 md:hidden">
                                    <div className="absolute inset-0 bg-black/60" onClick={() => setMobileMenuSong(null)} />
                                    <div className="absolute bottom-0 left-0 right-0 bg-neutral-900 rounded-t-2xl p-6 animate-slideUp">
                                        <div className="flex items-center gap-4 mb-6">
                                            <img src={mobileMenuSong.cover_medium || mobileMenuSong.cover} className="w-16 h-16 rounded-lg" />
                                            <div>
                                                <p className="font-bold">{mobileMenuSong.title}</p>
                                                <p className="text-sm text-gray-400">{mobileMenuSong.artist?.name}</p>
                                            </div>
                                        </div>
                                        <button
                                            onClick={() => { toggleFavorite(mobileMenuSong); setMobileMenuSong(null); }}
                                            className="w-full text-left py-3 border-b border-neutral-800"
                                        >❤️ Agregar a favoritos</button>
                                        <button
                                            onClick={() => { setSelectedSongForPlaylist(mobileMenuSong); setMobileMenuSong(null); }}
                                            className="w-full text-left py-3 border-b border-neutral-800"
                                        >➕ Agregar a playlist</button>
                                        {isPlaylistView && (
                                            <button
                                                onClick={() => { onRemoveSong(mobileMenuSong.codigo_unico); setMobileMenuSong(null); }}
                                                className="w-full text-left py-3 text-red-500"
                                            >❌ Quitar de lista</button>
                                        )}
                                    </div>
                                </div>
                            )}
                            ...
                            <p className="text-xs sm:text-sm text-gray-500 truncate mt-1">{artistName}</p>
                        </div>
                    );
                })}
            </div>
